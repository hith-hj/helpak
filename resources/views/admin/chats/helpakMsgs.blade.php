                    ...
                    <table class="table table-hover" id="grid-table5e7cd3e221e49">
                        <thead>
                            <tr>
                                ...
                                <th class="column-id">ID<a class="fa fa-fw fa-sort" href="http://127.0.0.1:8000/admin/auth/users?_pjax=%23pjax-container&amp;_sort%5Bcolumn%5D=id&amp;_sort%5Btype%5D=desc"></a>
                                </th>
                                <th class="column-name">Sender userName</th>
                                <th class="column-roles">full Name</th>
                                <th class="column-roles">Age</th>
                                <th class="column-name">Gender</th>
                                <th class="column-name">Image</th>
                                <th class="column-name">Message</th>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="column-id">{{$user->sender->id}}</td>
                                <td class="column-username">{{$user->sender->name}}</td>
                                <td class="column-name">{{$user->sender->firstName}}{{$user->sender->lastName}}</td>
                                <td class="column-name">{{$user->sender->age}}</td>
                                <td class="column-name">{{$user->sender->gender}}</td>
                                <td class="column-name">{{$user->sender->image}}</td>
                                <td class="column-name">
                                    <span class="label label-success">{{$user->message}}</span>
                                </td>
                                <td class="column-created_at">{{$user->created_at->diffForHumans()}}</td>
                                <td class="column-updated_at">{{$user->updated_at->diffForHumans()}}</td>
                                <td class="column-__actions__">
                                    <div class="grid-dropdown-actions dropdown" style="z-index:100;">
                                        <a href="/admin/userShow/{{$user->sender->id}}" style="padding:0 5px">
                                        <i class="fa fa-eye"></i>
                                        </a>
                                        <a href="#" data-toggle="modal" data-target="#User{{$user->id}}Delet" style="padding:0 5px">
                        ...
                        <div id="User{{$user->id}}Delet" class="modal fade"  tabindex="-1" role="dialog">
                            <div class="modal-dialog" >
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h3 class="modal-title" >Are you Sure you want to Delete this {{$user->title}}</h3>
                                    ...
                                    <div class="modal-footer">
                                        <a href="/admin/postDelete/{{$user->id}}" class="form-control-sm btn btn-danger" style="width:120px !important">Yes</a>
